...
fiws <- lapply(tabs,function(x) readxl::read_excel(path, sheet = x, skip = 0, col_names = F))%>%
  set_names(nm=tabs)

test <- map(fiws, .f = ~.x %>% janitor::remove_empty(which = c("cols","rows"))%>%
              ...
              bind_rows(.id = "Table")%>%
              mutate(Values = as.numeric(Values) * 1000, #data was origanally represented in thousands of dollars
